...
export const getPublicFundingContract = async () => {
  ...
  try {
    ...
    if (accounts.length === 0) {
      console.log("No accounts connected to Public Funding Contract. Requesting connection...");
      accounts = await window.ethereum.request({ method: "eth_requestAccounts" });
    }
    
    console.log("Connected account for Public Funding Contract:", accounts[0]);
    
    const provider = new BrowserProvider(window.ethereum);
    const signer = await provider.getSigner();
    
    // Log contract address for debugging
    console.log("Public Funding Contract address:", contractAddress);
    
    return new Contract(contractAddress, publicFundingContractABI, signer);
  } catch (error) {
    console.error("Error initializing Public Funding Contract:", error);
    if (error.code === 4001) {
